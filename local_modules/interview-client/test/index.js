...
describe('QuickChat Client', () => {
  ...
  describe('Using admin privileges', () => {
    parallel('', () => {
      it('should emit QuickChatClient#memberLeft for Alice', (done) => {
        await(aliceClient, 'memberLeft', done, (name) => {
          return name === bobName;
        });
      });
      ...
      it('should return a fulfilled promise', (done) => {
        setTimeout(() => {
          aliceClient.authenticate()
            .then(() => aliceClient.removeMember(bobName))
            .then(() => done());
        });
      });
    });
